...
def slide_inference(model, im, crop_size, stride, isOn=True):
    """
...
def inference(model,
    ...
    if not is_slide:
        ...
        logit = slide_inference(model, im, crop_size=crop_size, stride=stride, isOn=isOn)
    if not isOn:
        return None
    if hasattr(model, 'data_format') and model.data_format == 'NHWC':
